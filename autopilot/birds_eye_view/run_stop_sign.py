    ...
    def tick(self, vehicle):
        ...
        if self.target_stop_sign is None:
            ...
            if not self.stop_completed:
                ...
                stop_loc = self.target_stop_sign.get_transform().transform(
                    self.target_stop_sign.trigger_volume.location
                )
                if (
                    current_speed < self._speed_threshold
                    and stop_loc.distance(ev_loc) < 4
                ):
    ...
    def is_affected_by_stop(self, vehicle_loc, stop, multi_step=80):
        ...
        for actor_location in list_locations:
            if self.point_inside_boundingbox(
                actor_location, transformed_tv, stop_extent
            ):
                affected = True
