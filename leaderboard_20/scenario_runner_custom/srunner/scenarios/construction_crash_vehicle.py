...
class ConstructionObstacle(BasicScenario):
    ...
    def _initialize_actors(self, config):
        ...
        side_lane_wp = (
            self._construction_wp.get_left_lane()
            if self._direction == "right"
            else self._construction_wp.get_right_lane()
        )
        scenario_name = (
            "ConstructionObstacle"
            if side_lane_wp.lane_id * self._construction_wp.lane_id > 0
            else "ConstructionObstacleTwoWays"
        )
        CarlaDataProvider.active_scenarios.append(
            (
                scenario_name,
                [traffic_warning, last_cone, self._direction, False, 1e9, 1e9, False],
            )
        )  # added
