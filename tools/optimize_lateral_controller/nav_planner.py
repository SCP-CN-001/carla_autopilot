...
class RoutePlanner:
    def __init__(
        self,
        distance_between_route_points=0.1,
        times_supersample=10,
        append_meters_to_provided_route=50,
    ):
    ...
    def run_step(self, pos):
        ...
        self.route_idx += np.argmin(
            np.linalg.norm(
                pos[None, :2] - self.route_np[self.route_idx : until, :2], axis=1
            )
        )
    ...
    def set_route(
        ...
        if (
            starts_with_parking_exit and vehicle_loc is not None
        ):  # workaraound for ParkingExit scenario
            self.route_idx = 0
            self.last_route_idx = 0
        else:
        ...
        if (
            ...
            cmds = (
                ...
                + [cmd for _, cmd in global_plan]
                + [RoadOption.LANEFOLLOW] * self.append_meters_to_provided_route
            )
        else:
            cmds = (
                [RoadOption.LANEFOLLOW] * self.append_meters_to_provided_route
    ...
    def compute_rotation_angles(self, route_np):
        ...
        yaws = np.arctan2(diff[:, 1], diff[:, 0]) * 180.0 / np.pi
    ...
    def smooth_and_supersample(self, route_np, cmds):
        ...
        l_cmds = [
            cmds[idx]
            for idx in np.minimum(np.round(indices / 10), len(cmds) - 1).astype("int")
        ]
    ...
    def compute_distances_to_traffic_lights(self, carla_world, carla_map):
        ...
        for i in range(len(self.route_np) - 1, -1, -1):
            ...
            self.distance_to_next_traffic_light[i] = float(distance_idx) / 10.0

        self.distance_to_next_traffic_light = np.concatenate(
            [self.distance_to_next_traffic_light[:-40], 40 * [np.inf]]
        )
    ...
    def compute_distances_to_stop_signs(self, carla_world, carla_map):
        ...
        self.distance_to_next_stop_sign = np.full(
            self.route_np.shape[0], np.inf, dtype=np.float
        )
        ...
        if _list_stop_signs:
            ...
            for i in range(self.distance_to_next_stop_sign.shape[0] - 1, -1, -1):
                ...
                self.distance_to_next_stop_sign[i] = float(distance_idx) / 10.0
    ...
    def compute_curvature_dependent_speeds_limits(self, carla_world, carla_map):
        ...
        # n = 40
        self.curvature_dependent_speed = (
            120 / 3.6 * np.ones(self.route_np.shape[0], dtype=np.float)
        )
    ...
    def compute_leading_vehicles(self, list_vehicles, ego_vehicle_id):
        ...
        if len(vehicle_ids) and self.route_idx != self.route_np.shape[0]:
            ...
            distances = (
                ...
                - self.route_np[None, self.route_idx : self.route_idx + 800, :2][
                    :, ::10, :
                ]
            )
            ...
            rotation_angles = self.rotation_angles[
                self.route_idx : self.route_idx + 800
            ][::10]
    ...
    def compute_vehicles_behind(self, list_vehicles, ego_vehicle_id):
        ...
        if len(vehicle_ids) and self.route_idx != 0:
            ...
            distances = (
                vehicle_locations[:, None, :2]
                - self.route_np[None, from_idx : self.route_idx, :2][:, ::10, :]
            )
            distances = np.linalg.norm(distances, axis=2)
            route_indices = distances.argmin(axis=1)
            distances = distances.min(axis=1)
