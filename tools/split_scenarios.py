...
argparser.add_argument(
    ...
    type=str,
    help="Input path of the route that should be split.",
)
argparser.add_argument(
    "--save-path",
    default="../data/training_1_scenario",
    type=str,
...
argparser.add_argument(
    ...
    default=1,
    type=int,
    help="Maximum number of scenarios per xml file.",
)
argparser.add_argument(
    "--only-waypoints",
    default=False,
    action="store_true",
    help="Controls if scenarios are included or only waypoints.",
)
argparser.add_argument(
    "--exclude-scenarios",
    default=[],
    type=str,
    nargs="+",
    help="Specify, which scenarios should be excluded.",
)
...
class Route:
    ...
    def __init__(self, route_tree):
        ...
        self.is_junction = np.array(
            [
                carla_map.get_waypoint(
                    carla.Location(x=x[0], y=x[1], z=x[2])
                ).is_junction
                for x in self.trace
            ]
        )
    ...
    def sort_scenarios_in(self):
        for scenario, scenario_location in zip(
            self.scenarios, self.scenario_trigger_points
        ):
            diff = np.linalg.norm(self.trace - scenario_location[None], axis=1)
            diff[self.trace_type == "waypoint"] = 1e9
            min_idx = np.argmin(diff)

            self.trace_cmds = np.concatenate(
                [
                    self.trace_cmds[:min_idx],
                    [RoadOption.LANEFOLLOW],
                    self.trace_cmds[min_idx:],
                ]
            )
            self.trace = np.concatenate(
                [self.trace[:min_idx], [scenario_location], self.trace[min_idx:]]
            )
            self.trace_type = np.concatenate(
                [self.trace_type[:min_idx], ["scenario"], self.trace_type[min_idx:]]
            )
            self.trace_elem = (
                self.trace_elem[:min_idx] + [scenario] + self.trace_elem[min_idx:]
            )
    ...
    for i in range(first_scenario_idx - 1, -1, -1):
        ...
        if distances[i] > min_distance:
            location = carla.Location(
                x=route.trace[i][0], y=route.trace[i][1], z=route.trace[i][2]
            )
...
def get_distance_to_end_of_next_junction(route, scenario_idx):
    ...
    while (
        scenario_idx + end_idx_of_next_junction < len(is_junction)
        and is_junction[end_idx_of_next_junction]
    ):
        end_idx_of_next_junction += 1

    differences = np.diff(
        route.trace[scenario_idx : scenario_idx + end_idx_of_next_junction, :2], axis=0
    )
    distance = (
        np.linalg.norm(differences, axis=1).sum() + 25
    )  # plus 20 to make sure we are outside the junction
...
def get_distance_to_end_of_next_junction_with_traffic_lights(route, scenario_idx):
    ...
    while scenario_idx + end_idx_of_next_junction < len(is_junction):
        if is_junction[end_idx_of_next_junction]:
            ...
            if (
                wp.is_junction
                and len(world.get_traffic_lights_in_junction(wp.junction_id)) > 0
            ):
                break
    ...
    distance = (
        np.linalg.norm(differences, axis=1).sum() + 25
    )  # plus 25 to make sure we are outside the junction
...
def get_distance_to_lane_change(route, scenario_idx):
    ...
    while True:
        if (
            idx >= len(route.trace) - 1
            or route.trace_cmds[idx] == RoadOption.CHANGELANELEFT
            or route.trace_cmds[idx] == RoadOption.CHANGELANERIGHT
        ):
            break
        idx += 1
...
def get_distance_between_overlapping_routes(route, scenario_idx, max_distance):
    ...
    start_actor_flow, end_actor_flow = scenario_elem.find(
        "start_actor_flow"
    ), scenario_elem.find("end_actor_flow")

    source_wp = carla_map.get_waypoint(
        carla.Location(
            x=float(start_actor_flow.attrib["x"]),
            y=float(start_actor_flow.attrib["y"]),
            z=float(start_actor_flow.attrib["z"]),
        )
    )
    sink_wp = carla_map.get_waypoint(
        carla.Location(
            x=float(end_actor_flow.attrib["x"]),
            y=float(end_actor_flow.attrib["y"]),
            z=float(end_actor_flow.attrib["z"]),
        )
    )
    ...
    interpolated_traces = [
        grp_1.trace_route(x.transform.location, y.transform.location)
        for (x, y) in zip(source_wps, sink_wps)
    ]
    ...
    while True:
        ...
        if (
            idx >= len(route.trace)
            or current_distance < max_distance
            or min_distance + 20 < current_distance
        ):
            break
        distances = np.linalg.norm(
            route.trace[idx][None, :] - interpolated_traces, axis=1
        )
        idx += 1
...
def get_distance_between_overlapping_routes_v2(route, scenario_idx, max_distance):
    ...
    source_loc = carla.Location(
        x=float(start_actor_flow.attrib["x"]),
        y=float(start_actor_flow.attrib["y"]),
        z=float(start_actor_flow.attrib["z"]),
    )
    sink_loc = carla.Location(
        x=float(end_actor_flow.attrib["x"]),
        y=float(end_actor_flow.attrib["y"]),
        z=float(end_actor_flow.attrib["z"]),
    )
    ...
    while True:
        ...
        if (
            ...
            break
        distances = np.linalg.norm(
            route.trace[idx][None, :] - interpolated_trace, axis=1
        )
        idx += 1
...
def get_distance_till_right_lane_change_is_possible(route, scenario_idx):
    ...
    while True:
        wp = carla_map.get_waypoint(
            carla.Location(
                x=route.trace[idx, 0], y=route.trace[idx, 1], z=route.trace[idx, 2]
            )
        )
        if (
            idx >= len(route.trace)
            or wp.lane_change == carla.LaneChange.Right
...
def get_succeeding_distance(route, scenario_idx):
    ...
    if scenario_type == "Accident" or scenario_type == "AccidentTwoWays":
        ...
        or scenario_type == "HazardAtSideLaneTwoWays"
    ):
        additional_distance = get_value_parameter(scenario_elem, "distance", 100)
        additional_distance += get_value_parameter(
            scenario_elem, "bicycle_drive_distance", 50
        )
    elif (
        scenario_type == "ConstructionObstacle"
        or scenario_type == "ConstructionObstacleTwoWays"
    ):
        additional_distance = get_value_parameter(scenario_elem, "distance", 100)
        ...
        additional_distance = get_distance_to_end_of_next_junction_with_traffic_lights(
            route, scenario_idx
        )
        ...
        additional_distance = get_distance_between_overlapping_routes(
            route, scenario_idx, 2
        )  # lane width is often 3.5m and distance between following waypoints is 1m
    elif (
        scenario_type == "MergerIntoSlowTraffic"
        or scenario_type == "MergerIntoSlowTrafficV2"
    ):
        additional_distance = get_distance_between_overlapping_routes_v2(
            route, scenario_idx, 1.5
        )  # lane width is often 3.5 and distance between following waypoints is 1m
    elif scenario_type == "HighwayCutIn":
        additional_distance = get_distance_till_right_lane_change_is_possible(
            route, scenario_idx
        )
...
def get_succeeding_waypoints(route_idx, last_scenario_idx):
    ...
    min_distance = get_succeeding_distance(
        route, last_scenario_idx
    )  # distance_after[scenario_type]
...
def write_begin_of_file(data, route, n_route):
    ...
    for param_name, value_begin, value_end in zip(
        route.weather_params, route.weather_values_begin, route.weather_values_end
    ):
...
for route_idx, route in enumerate(l_routes):
    for idx, trace_type in enumerate(route.trace_type):
        if (
            trace_type == "scenario"
            and route.trace_elem[idx].attrib["type"] not in args.exclude_scenarios
        ):
...
plt.suptitle(
    "Route lengths for {} scenarios per route (number of routes: {})".format(
        args.max_scenarios, len(route_lengths)
    )
)
...
with open(
    os.path.join(args.save_path, "average_route_lenghts_per_scenario.json"),
    "w",
    encoding="utf8",
) as fp:
