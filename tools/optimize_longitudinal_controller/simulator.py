...
class Simulator:
    ...
    def interpolate_trace(self):
        ...
        for i in range(self.track_points.shape[0] - 1):
            ...
            self.trace += self.grp.trace_route(
                from_wp.transform.location, to_wp.transform.location
            )
    ...
    def start_simulator(self):
        ...
        subprocess.Popen(
            "sh {} -RenderOffScreen -carla-streaming-port=0 -carla-rpc-port={} -nosound &".format(
                self.carla_simulator_path, self.port
            ),
            shell=True,
        )
        time.sleep(10)
    ...
    def spawn_car(self):
        ...
        if self.ego_vehicle_bp.has_attribute("color"):
            color = np.random.choice(
                self.ego_vehicle_bp.get_attribute("color").recommended_values
            )
            self.ego_vehicle_bp.set_attribute("color", color)
        if self.ego_vehicle_bp.has_attribute("driver_id"):
            driver_id = np.random.choice(
                self.ego_vehicle_bp.get_attribute("driver_id").recommended_values
            )
        ...
        if self.ego_vehicle_bp.has_attribute("speed"):
            self.player_max_speed = float(
                self.ego_vehicle_bp.get_attribute("speed").recommended_values[1]
            )
            self.player_max_speed_fast = float(
                self.ego_vehicle_bp.get_attribute("speed").recommended_values[2]
            )
    ...
    def get_steer_stanley(
        ...
        n_lookahead = np.clip(
            speed_scale * current_speed + speed_offset, 0, 105
        ).astype("int")
        ...
        yaw_cross_track = np.arctan2(
            ego_vehicle_location[1] - route_np[0, 1],
            ego_vehicle_location[0] - route_np[0, 0],
        )
        yaw_diff_of_path_cross_track = yaw_path - yaw_cross_track
        yaw_diff_of_path_cross_track = self.normalize_angle(
            yaw_diff_of_path_cross_track
        )

        crosstrack_error = np.linalg.norm(ego_vehicle_location - route_np[0])
        crosstrack_error = (
            np.abs(crosstrack_error)
            if yaw_diff_of_path_cross_track > 0
            else -np.abs(crosstrack_error)
        )
    ...
    def get_steer_pure_persuit(
        self, route_np, current_speed, ego_vehicle_location, ego_rotation, params
    ):
        (
            car_length,
            speed_scale,
        ...
        n_lookahead = int(
            min(
                np.clip(speed_scale * current_speed + speed_offset, 0, 105),
                route_np.shape[0] - 1,
            )
        )
        ...
        rear_axle = ego_vehicle_location[:2] - np.array(
            [
                np.cos(ego_rotation) * car_length / 2.0,
                np.sin(ego_rotation) * car_length / 2,
            ]
        )
        ...
        steering = np.clip(steer_scale * steering, -1.0, 1.0).item()
    ...
    def get_steer_pid(
        ...
        steering = np.clip(
            k_p * heading_error + k_d * derivative + k_i * integral, -1.0, 1.0
        ).item()
    ...
    def get_steer(self, route_np, current_speed, vehicle_pos_np, ego_rotation):
        ...
        return self.get_steer_pid(
            route_np, current_speed, vehicle_pos_np, ego_rotation, params
        )
    ...
    def get_throttle(self, route_idx, current_speed, target_speed, params):
        ...
        return np.clip(features @ params[:-1], 0.0, 1.0)
    ...
    def drive_track(self, params):
        ...
        self.ego_vehicle = self.world.try_spawn_actor(
            self.ego_vehicle_bp, start_transform
        )
        self.world.tick()
        ...
        while True:
            ...
            route_np, _, _, _, _, _, _, _, _, _ = self.route_planner.run_step(
                vehicle_pos_np
            )
            ...
            if (
                self.route_planner.route_idx < 1270
                and self.route_planner.route_idx > 1190
                or self.route_planner.route_idx < 1920
                and self.route_planner.route_idx > 1840
                or self.route_planner.route_idx < 6750
                and self.route_planner.route_idx > 6570
            ):
                distance = 0
            ...
            steer = self.get_steer(
                route_np, current_speed, vehicle_pos_np, ego_rotation
            )
            throttle = self.get_throttle(route_idx, current_speed, target_speed, params)

            throttle_control, brake_control = np.clip(throttle, 0.0, 1.0), np.clip(
                -throttle, 0.0, 1.0
            )
            self.ego_vehicle.apply_control(
                carla.VehicleControl(
                    throttle=throttle_control, steer=steer, brake=brake_control
                )
            )
            ...
            self.recording_dict["forward_vectors"].append(
                [acceleration.x, acceleration.y, acceleration.z]
            )
            self.recording_dict["accelerations"].append(
                [acceleration.x, acceleration.y, acceleration.z]
            )
            ...
            if no_movement_counter >= 30 or distance > 4.0:
                l_speed, l_target_speed = [100.0], [0.0]
                break
        ...
        diff = np.abs(l_speed - l_target_speed).clip(0.0, 1.0)
        score = 1.0 if len(diff) == 1 else diff.mean()
        ...
        print(
            f"Score: {score}\tBest score: {self.best_score}\tBest params: {self.best_parameters}"
        )
