...
def compute_speed_limit_map(carla_port, map_name):
    ...
    def compute_speed_limits(wps, speed_limit_indices):
        for wp, speed_limit_idx in zip(wps, speed_limit_indices):
            ...
            for dist in range(300, 4000, 300):
                if (
                    len(wp.get_landmarks_of_type(dist, carla.LandmarkType.MaximumSpeed))
                    > 0
                ):
            ...
            if found_max_distance:
                while True:
                    ...
                    landmarks = wp.get_landmarks_of_type(
                        mid_distance, carla.LandmarkType.MaximumSpeed
                    )
    ...
    if (
        len(speed_signs_locs) == 0
    ):  # use 50 km/h as default speed if the map contains no speed sign
        speed_limits[:] = 50.0
    else:
    ...
    map_speed_limits[
        rounded_map_waypoints_locs[:, 0], rounded_map_waypoints_locs[:, 1]
    ] = speed_limits
    mask = speed_limits == -1
    ...
    for _ in tqdm(range(unassigned_waypoints_locs.shape[0])):
        ...
        while True:
            ...
            indices = np.concatenate(
                [x.flatten()[None], y.flatten()[None]], axis=0
            ).T  # [9, 2]
            indices = (
                unassigned_waypoints_locs[:, None, :] + indices[None, :, :]
            )  # [13361, 9, 2]
            indices[:, :, 0] = indices[:, :, 0].clip(0, map_size[0] - 1)
            indices[:, :, 1] = indices[:, :, 1].clip(0, map_size[1] - 1)
            close_speed_limits = map_speed_limits[
                indices[:, :, 0], indices[:, :, 1]
            ]  # [13361, 9]
        ...
        map_speed_limits[
            unassigned_waypoints_locs[max_idx, 0], unassigned_waypoints_locs[max_idx, 1]
        ] = max_speed_limit
        unassigned_waypoints_locs = np.delete(
            unassigned_waypoints_locs, max_idx, axis=0
        )
    ...
    for i in tqdm(range(rounded_map_waypoints_locs.shape[0])):
        ...
        filtered_speed_limit = map_speed_limits[
